...
(function ($)
{
    ...
    $.ligerui.controls.CheckBoxList.ligerExtend($.ligerui.controls.Input, {
        ...
        setRowSize: function (rowSize)
        {
            var rs = window.parseInt(rowSize);
            if (isNaN(rs) || rs < 1 || rs == this.options.rowSize)
                return;
            this.options.rowSize = rs;
            this.refresh();
        },
        refresh:function()
        {
            ...
            for (var i = 0; i < data.length; i++)
            {
                ...
                out.push("<td><input type='checkbox' name='" + name + "' value='" + val + "' id='" + id + "'" + disabled + "/><label for='" + id + "'>" + txt + "</label></td>");
            }
            if (appendRowStart) out.push('</tr>');
...
(function ($)
{
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _setWidth: function (value)
        {
            ...
            var g = this, p = this.options;
            var oldWidth = g.wrapper.width();;
            g.wrapper.css({ width: value });
            value = g.wrapper.width();
            if (value > 20)
            {
...
(function ($)
{
    ...
    $.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input, {
        ...
        _setWidth: function (value)
        {
            ...
            var oldWith = g.textwrapper.width();
            g.textwrapper.css({ width: value });
            value = g.textwrapper.width();
            if (value > 20)
            {
            ...
            {
                g.textwrapper.css({ width: oldWith });
            }
        },
        _setCss : function(css)
        {
            if (css) {
                this.text.addClass(css);
            } 
        },
        _setValue: function (value)
        {
        ...
        _applyEditor: function (obj)
        {
            ...
            if (column.editor.type && p.editors[column.editor.type])
            {
                ...
                var editor = p.editors[column.editor.type],
                    ...
                    //var grid = jcell.parents(".l-grid");
                    //left -= grid.scrollLeft();
                    //top -= grid.scrollTop();
                    //console.log(grid);
                    //console.log(grid.scrollLeft());
                    //console.log(grid.scrollTop());
                //console.log(rowcell.offsetLeft);
                    //console.log(g.gridbody[0].offsetLeft);
                //console.log(g.gridbody);
                //console.log(g.gridview2);
...
(function ($)
{
    ...
    $.ligerui.controls.TextBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _setCss: function (value) {
            this.wrapper.addClass(value);//.css(value);
        },
        _setWidth: function (value)
        {
            var oldWidth = this.wrapper.width();
            this.wrapper.css({ width: value });
            var value = this.wrapper.width();
            if (value > 20)
            {
                this.inputText.css({ width: value - 4 });
            }
            else {
                this.wrapper.css({ width: oldWidth });
            }
        },
        _setHeight: function (value)
        {
