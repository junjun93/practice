...
<tr role="row" class="user-name odd" data-id="{{id}}"><%--even--%>
    ...
    <td>{{mail}}</td>
    <td>{{telephone}}</td>
    <td>{{#blod}}{{showStatus}}{{/blod}}</td><%--此处套用函数对status做特殊处理--%>
    <td>
        <div class="hidden-sm hidden-xs action-buttons">
            <a href="#" class="green user-edit" data-id="{{id}}">
                <i class="ace-icon fa fa-pencil bigger-100"></i>
            </a>
            <a href="#" class="red user-acl" data-id="{{id}}">
...
<script type="application/javascript">
    $(function(){
        ...
        function renderUserListAndPage(result, url){
            ...
            var pageSize = $("#pageSize").val();
            var pageNo = $("#userPage .pageNo").val() || 1;
            renderPage(url, result.data.total, pageNo, pageSize, result.data.total > 0 ? result.data.data.length : 0, "userPage",
